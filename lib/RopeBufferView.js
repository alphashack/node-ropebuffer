...
p.seek = function (where) {
    if (0 > where || where > this._rb.length) {
        throw new Error("Cannot seek (to " + where + ") outside RopeBufferView [0;" + this._buffersLength + "].");
    }
...
[
    ...
    p['read' + name] = function (noAssert) {
        var index = this._rb.indexToBuffer(this._cursor),
            ret = index.buffer['read' + name](index.index, noAssert);
    ...
    p['write' + name] = function (value, noAssert) {
        var index = this._rb.indexToBuffer(this._cursor);
        index.buffer['write' + name](value, index.index, noAssert);
        this._cursor += bytes;
    };
